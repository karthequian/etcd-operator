...
func WaitBackupDeleted(kubeClient kubernetes.Interface, cl *spec.Cluster, checkerOpt StorageCheckerOptions) error {
	retries := 5
	if checkerOpt.CompletelyGone {
		// TODO: revisit this later when we use "background propagate" deletion policy.
		retries = 60
	}
	err := retryutil.Retry(5*time.Second, retries, func() (bool, error) {
		...
		if (!checkerOpt.CompletelyGone && d.DeletionTimestamp != nil) || apierrors.IsNotFound(err) {
			return true, nil
		}
		if err == nil {
			return false, nil
